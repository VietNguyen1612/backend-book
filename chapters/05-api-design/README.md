...
### [5.4 WebAuthn & Passkeys](webauthn-and-passkeys.md)

Passwordless, phishing-resistant authentication built on public-key cryptography. Covers the WebAuthn/FIDO2 model, the registration (attestation) and authentication (assertion) ceremonies, passkeys (synced vs device-bound), usernameless login, and the operational gotchas that bite teams in production.

**Key topics:** WebAuthn/FIDO2, asymmetric per-site credentials, attestation and assertion ceremonies, py_webauthn and @simplewebauthn/browser, discoverable credentials, synced vs device-bound passkeys, conditional-UI autofill, phishing resistance via origin binding, sign-counter clone detection, account recovery, RP ID scoping
